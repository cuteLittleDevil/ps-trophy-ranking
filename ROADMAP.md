...
v1 排行榜页面已完成。支持真实 PSN 档案拉取与模拟数据灌入，包含完整的入榜、查找、分页、排序、持久化功能。
...
- 2026-09-11: 重构架构
  ...
  - 去掉 `PSN_MODE` 配置，服务默认真实 PSN 入榜
  - 模拟用户 Online ID：`simulation` + 随机数字，查重保证 UNIQUE
  - 头像占位：A–Z 随机字母，模板支持 `letter:` 前缀显示圆形字母头像
  - 更新文档：删除 fixture 决策，写清 seed 接口契约与追加语义
