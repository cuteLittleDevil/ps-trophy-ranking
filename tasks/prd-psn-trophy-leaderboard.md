...
v1 明确不做账号密码登录，也不做上万并发。Sony 没有对个人项目开放的官方账号授权，真实档案可能拉不到或随时中断，因此必须提供 **管理灌数接口**，用于压测和演示，保证无凭证时也能跑通本地演示。

> **架构更新 (2026-09-11)**: 已删除 fixture 演示模式，改用 `/admin/seed` HTTP 管理接口追加模拟用户。US-005/FR-13 改为灌数接口需求。
...
### US-005: 管理灌数接口追加模拟用户
**Description:** As a 开发者/压测者, I want HTTP 管理接口追加模拟用户 so that 没凭证时也能演示入榜，以及为压测填充数据。

**Acceptance Criteria:**
- [ ] 提供 `POST /admin/seed` 接口，参数 `count`（正整数）
- [ ] 每次追加，不清空数据库；生成 Online ID 查重保证 UNIQUE
- [ ] Online ID：`sim` + 定宽数字（总长度 3–16 字符）
- [ ] 奖杯数：合理随机（铜 0–5000、银 0–2000、金 0–800、白金 0–200）
- [ ] 积分：使用 `rank.Score` 计算
- [ ] 头像：A–Z 随机字母占位
- [ ] `count` 硬上限（例如 1000），超限返回 400
- [ ] 非法/≤0 count 返回 400
- [ ] 只接受回环地址（127.0.0.1/::1），非回环返回 403
- [ ] 响应 JSON：`{"ok":true,"inserted":N}` 或 `{"ok":false,"error":"..."}`
- [ ] 自动化测试覆盖：合法追加、ID 前缀与长度、非法 count、超上限、累加语义、非回环拒绝
- [ ] Verify in a browser（灌数后看到模拟用户）
...
- [ ] 同一 Online ID 同步冷却 15 分钟；冷却期内提示「同步过于频繁」并保留旧数据
- [ ] 不把凭证、会话令牌写入日志或页面
- [ ] 自动化测试覆盖上述失败路径，且不依赖真实外网
- [ ] Verify in a browser（至少验证未配置凭证失败态；真实 PSN 取决于是否配置凭证）
...
- FR-13: 系统必须提供 `/admin/seed` HTTP 管理灌数接口，追加模拟用户（压测/演示用），只接受回环地址。
...
- FR-21: 系统必须在 README 写出启动命令、灌数接口示例、以及（可选）配置真实 PSN 凭证的方式。
...
- 未配置 PSN 凭证时，按 README 用灌数接口能完成：看空榜 → seed 追加模拟用户 → 看榜显示 → 翻页（若不足 50 人则验证只有 1 页）→ 查找命中高亮 → 查找未入榜 ID
