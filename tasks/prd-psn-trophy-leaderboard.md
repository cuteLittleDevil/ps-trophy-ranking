...
实现方案见 `tasks/spec-psn-trophy-leaderboard.md` 。本文件只描述产品行为，不指定语言、框架、库或存储引擎。

> **实现状态 (2026-09-11)**：v1 已合入 main（`4bde179`，PR #1–#10）。下列验收标准均已落地；提交链见 `提交历史说明.md`。
...
**Acceptance Criteria:**
- [x] 按 README 的启动方式在本地打开排行榜页，默认访问地址写在 README
- [x] 打开首页看到中文标题、入榜表单、排行榜表格表头
- [x] 榜上无人时显示明确空态文案（例如「还没有玩家入榜」），不显示空白坏表
- [x] 项目可构建
...
**Acceptance Criteria:**
- [x] `score = bronze*15 + silver*30 + gold*90 + platinum*300`
- [x] 排序键依次为：score 降序、platinum 降序、gold 降序、silver 降序、bronze 降序、online_id 升序
- [x] 名次为竞赛名次：同分同名次，下一名次跳过（例：1、1、3）
- [x] 自动化测试覆盖：普通分差、完全同分、仅白金不同、空奖杯全 0
- [x] 相关自动化测试通过
...
**Acceptance Criteria:**
- [x] 玩家数据重启进程后仍在（不依赖进程内存）
- [x] 每页 50 人；第 2 页第一条的全局名次接续，而不是从 1 重计
- [x] 每一行展示：名次、头像（无头像用占位图）、PSN Online ID、白金/金/银/铜数量、奖杯积分
- [x] 页码超出范围时显示空列表和「没有更多玩家」，不出现服务错误页
- [x] 相关自动化测试通过
- [x] Verify in a browser
...
**Acceptance Criteria:**
- [x] 首页有入榜表单，字段为 PSN Online ID
- [x] Online ID 校验：去首尾空格后长度 3–16，只允许字母、数字、`-`、`_`；非法时表单下显示中文错误，不向外同步奖杯
- [x] 空提交显示「请输入 PSN Online ID」，不向外同步奖杯
- [x] 同一 Online ID 再次入榜：更新奖杯数字和积分，不新增行；保留首次入榜时间，更新「最近同步时间」
- [x] 入榜成功后跳到该玩家所在页，并高亮其行
- [x] 已入榜的真实玩家可在该行点「刷新」更新奖杯（与入榜共享 15 分钟冷却；`sim` 前缀模拟用户不显示按钮）
- [x] 相关自动化测试通过
- [x] Verify in a browser
...
**Acceptance Criteria:**
- [x] 提供 `POST /admin/seed` 接口，参数 `count`（正整数）
- [x] 每次追加，不清空数据库；生成 Online ID 查重保证 UNIQUE
- [x] Online ID：`sim` + 定宽数字（总长度 3–16 字符）
- [x] 奖杯数：合理随机（铜 0–5000、银 0–2000、金 0–800、白金 0–200）
- [x] 积分：使用 `rank.Score` 计算
- [x] 头像：A–Z 随机字母占位
- [x] `count` 硬上限（例如 1000），超限返回 400
- [x] 非法/≤0 count 返回 400
- [x] 只接受回环地址（127.0.0.1/::1），非回环返回 403
- [x] 响应 JSON：`{"ok":true,"inserted":N}` 或 `{"ok":false,"error":"..."}`
- [x] 自动化测试覆盖：合法追加、ID 前缀与长度、非法 count、超上限、累加语义、非回环拒绝
- [x] Verify in a browser（灌数后看到模拟用户）

### US-006: 真实公开档案拉取与失败态
**Description:** As a 玩家, I want 在配置了服务端凭证后用真实公开档案入榜，并在失败时看到原因 so that 不是一个只能播假数据的玩具。

**Acceptance Criteria:**
- [x] 真实模式下，系统按 Online ID 拉公开奖杯汇总；凭证只存在于运行配置，不进仓库
- [x] 用户不存在：页面显示「找不到该 PSN 用户」
- [x] 档案隐私不允许查看奖杯：显示「该用户奖杯未公开，无法入榜」
- [x] 未配置凭证：显示「服务端未配置 PSN 凭证」，不出现服务错误页
- [x] 上游限流或网络错误：显示「暂时无法同步奖杯，请稍后重试」
- [x] 同一 Online ID 同步冷却 15 分钟；冷却期内提示「同步过于频繁」并保留旧数据
- [x] 不把凭证、会话令牌写入日志或页面
- [x] 自动化测试覆盖上述失败路径，且不依赖真实外网
- [x] Verify in a browser（至少验证未配置凭证失败态；真实 PSN 取决于是否配置凭证）

### US-007: 按 PSN ID 查找并跳到我的排名
**Description:** As a 访客, I want 输入 PSN ID 定位到该行 so that 人多时不用翻页找自己。

**Acceptance Criteria:**
- [x] 榜页有查找框，提交后若在榜中：跳到所在页并高亮该行
- [x] 若 ID 合法但不在榜中：显示「该玩家尚未入榜」，不出现找不到页面的白屏
- [x] 入榜成功后记住该 Online ID（刷新页面后仍可用）；点击「我的排名」等价于查找该 ID
- [x] 没有记住的 ID 时，「我的排名」提示「先入榜或先查找」
- [x] 查找非法 ID 时复用 US-004 的校验文案
- [x] 相关自动化测试通过
- [x] Verify in a browser
