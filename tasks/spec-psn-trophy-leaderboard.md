...
| `internal/memrank` | 全量内存排行榜 + Top1000 视图；门槛分；当前 RWMutex；**Left-Right 双数组已批准待实现（§13.10）** | 直连 PSN |
...
- **memrank Left-Right 双数组**（§13.10，已批准待实现）：暗面更新 → 原子切换读指针 → 亮面追平；读不经过长写锁
...
- **memrank Left-Right 双数组**（✅ **已批准，待实现，2026-09-12**）：见 §13.10。目标先保证约百万用户读路径不被 `UpsertBatch` 写锁长时间阻塞；千万级整表双份不作为本阶段终局。
...
3. **Phase 2++ memrank Left-Right 双数组** ⏳ **已批准待实现（§13.10，2026-09-12）**
...
### 13.10 memrank Left-Right 双数组（已批准，待实现）

**状态**：✅ **已批准（2026-09-12）**，尚未实现。本阶段目标：**约百万用户**下，排行榜读路径不被内存合并的写锁长时间阻塞；**千万级整表双份**不作为本阶段终局（内存与双倍合并成本过高，后续另议树/分片/Top-N 分层）。
